...
export default function AuditAnalyzingPage() {
  const searchParams = useSearchParams();
  const siteParam = searchParams.get("site") ?? "";
  const siteLabel = useMemo(() => normalizeSiteLabel(siteParam), [siteParam]);
        ...
        <main className="mx-auto flex min-h-[calc(100vh-7rem)] max-w-7xl flex-col items-center justify-center py-10 text-center">
          ...
          <div className="relative mt-16 w-full max-w-[56rem]">
            ...
            <div className="relative overflow-hidden rounded-[16px] border border-[#474A86] bg-[radial-gradient(circle_at_top,rgba(122,129,255,0.32),rgba(26,27,61,0.94)_50%,rgba(20,21,47,1)_100%)]">
              ...
              {/* Decorative glowing dots (always visible) */}
              <style>{`
                @keyframes dotFade {
                  0%, 100% { opacity: 0.92; }
                  50% { opacity: 0.32; }
              ...
              {/* Animated preview content (fades in/out, glow and dots remain visible) */}
              <div
                className="relative z-20"
                style={{
                  opacity: showPreview ? 1 : 0,
                  willChange: "opacity",
                  animation: !showPreview
                    ? undefined
                    : runPreviewFlash
                      ? "previewFlash 3.2s linear infinite"
                      : "previewIntro 0.45s ease-out 1",
                }}
              >
              ...
              {/* Always-visible inward glow above preview */}
